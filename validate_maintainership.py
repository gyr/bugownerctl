...
def load_config(config_file_path: str) -> Dict[str, Any]:
    try:
        ...
        print(f"Error: Invalid YAML format in '{config_file_path}': {e}", file=sys.stderr)
        sys.exit(1)
...
def get_package_name_from_repo_metadata(filename: str) -> Optional[str]:
    ...
    match_fallback: Optional[re.Match[str]] = re.match(r"(.*?)-\d+(-.*)?$", name_no_suffix)
    if match_fallback:
...
def get_binary_data_from_repo_metadata() -> List[Tuple[str, str, Optional[str]]]:
    """
    Parses repository metadata from .zst files.
    ...
    for zst_file in ZST_FILE_PATHS:
        ...
        try:
            ...
            with open(zst_file, 'rb') as compressed_file:
                with dctx.stream_reader(compressed_file) as xml_stream:
                    context = etree.iterparse(xml_stream, events=('end',), tag=f'{{{COMMON_NS_URI}}}package')
                    for _, element in context:
                        name_element = element.find(f'{{{COMMON_NS_URI}}}name')
                        sourcerpm_element = element.find('common:format/rpm:sourcerpm', namespaces=NSMAP)

                        if name_element is not None and name_element.text:
                            binary_name: str = name_element.text
                            sourcerpm_filename: str = sourcerpm_element.text if sourcerpm_element is not None and sourcerpm_element.text else 'N/A'
                            package_name: Optional[str] = get_package_name_from_repo_metadata(sourcerpm_filename)
                            binary_data_set.add((binary_name, sourcerpm_filename, package_name))
                        element.clear()
        except FileNotFoundError:
    ...
    sorted_binary_data: List[Tuple[str, str, Optional[str]]] = sorted(list(binary_data_set))
...
def get_binaries_from_productcompose() -> Optional[Set[str]]:
    ...
    try:
        ...
        if marker in content:
            ...
            print(f"Warning: Marker '{marker}' not found. Parsing the whole file.", file=sys.stderr)
...
def check_invalid_packages(binary_data_list: List[Tuple[str, str, Optional[str]]], submodule_list: List[str]) -> Optional[Set[str]]:
    ...
    checks them against a list of false positives, and queries OBS for the source package.
    Invalid packages are written to a file.
    """
    print("--- Checking for invalid packages ---")
    packages_from_repo: Set[str] = {item[2] for item in binary_data_list}
    ...
    try:
        ...
        print(f"Warning: False positives file not found at '{FALSEPOSITIVES_FILE}'. Using empty remapping.", file=sys.stderr)
    except json.JSONDecodeError:
        print(f"Error: Invalid JSON format in '{FALSEPOSITIVES_FILE}'. Using empty remapping.", file=sys.stderr)

    if remapping:
        # Apply remapping and removal
        print(f"--- Updating {len(remapping)} false-positive packages from {FALSEPOSITIVES_FILE} ---")
    ...
    unknow_packages: Set[str] = packages_from_repo - set(submodule_list)
    valid_packages: Set[str] = packages_from_repo - unknow_packages

    invalid_packages: List[str] = []
    false_positve_packages: Dict[str, str] = {}

    print(f"Found {len(unknow_packages)} unknow packages.")
    for package in unknow_packages:
        if package is None:
            continue
        obs_package = get_source_package_from_obs(package)
        if obs_package:
            valid_packages.add(obs_package)
            false_positve_packages[package] = obs_package
        else:
            invalid_packages.append(package)

    if false_positve_packages:
        print(f"Found {len(false_positve_packages)} false-positives packages.")
        # Update false-positives file
        remapping.update(false_positve_packages)
...
def main() -> None:
    ...
    binary_data_list: List[Tuple[str, str, Optional[str]]] = get_binary_data_from_repo_metadata()
    if not binary_data_list:
        print("Could not gather any package data from XML files. Aborting.", file=sys.stderr)
        return
